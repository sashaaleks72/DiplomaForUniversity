...
const Comments = (): JSX.Element => {
    ...
    const [rate, starsSrcs, onStarMouseEnter, onStarClick, onStarMouseLeave, setRate] = useCommentRate(5);

    useEffect(() => {
        const init = async () => {
            const recievedComments = teapot != null ? await TeapotsService.getCommentsByProductId(teapot.id!) : [];
    ...
    const addNewComment = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (teapot) {
            ...
            newComment.advantages = comment.advantages;
            newComment.disadvantags = comment.disadvantags;
            newComment.rate = rate;
            newComment.productId = teapot.id!;
            newComment.userName = "Oleksandr Tretiakov";

            await TeapotsService.postUserComment("379c8eb9-d017-4cca-8b2e-a9cb3a77acc1", newComment);
        ...
        <div className="ms-2 mt-2 position-relative d-flex justify-content-between">
            <div className="w-50">
                <div className="border border-grey rounded-1 d-flex align-items-center justify-content-between p-3">
                    ...
                    <div className="btn btn-success" onClick={() => setIsModal(true)}>
                        Write a comment
                    </div>
                </div>
                <div className="mt-2">
                    {comments.map((comment, index) => {
                        return (
                            <div key={index} className="mb-3">
                                <div className="fw-bold fs-5">{comment.userName}</div>
                                <div>
                                    {Array.from(Array(5).keys()).map((el, index) => (
                                        <img
                                            key={index}
                                            src={index < comment.rate ? filled_star : unfilled_star}
                                            height={25}
                                        />
                                    ))}
                                </div>
            ...
            <div className="max-w-50 align-self-start position-sticky top-0 pt-2">
                <div className="border border-grey rounded-1">
                    <div className="d-flex">
                        <img className="p-2" src={teapot?.imgName} height={150} />
                        <div className="p-2">
                            <div>{teapot?.name}</div>
                            <div className="d-flex align-items-center">
                                ...
                                <div
                                    ...
                                    onClick={() => setIsHeartPressed((prev) => !prev)}>
                                    {isHeartPressed ? (
            ...
            <Modal isVisible={isModal} setIsVisible={setIsModal}>
                <div className="position-relative">
                    <img
                        ...
                        height={20}
                    />
                    <h2 className="fs-4 fw-light">Write comment about the product: </h2>
                    <form onSubmit={(e) => addNewComment(e)}>
                        ...
                        <div className="mt-1">
                            <span className="fw-light fs-5">Disadvantages: </span>
                            <input
                                value={comment.disadvantags}
                                onChange={onFormChangeHandler}
                                name="disadvantags"
                                type="text"
                                className="form-control"
                                placeholder="Write disadvantages"></input>
                        </div>
